function mySkills(x) {
  if (x.matches) {
    const Texts = ['WORDPRESS','HTML5', 'CSS3', 'JAVASCRIPT', 'REACT', 'C++', 'PYTHON', 'SASS', 'NODEJS', 'MONGODB', 'EXPRESS', 'MYSQL', 'JQUERY', 'JAVA','BOOTSTRAP','TAILWIND-CSS'];
...
function navSlide() {
  ...
  // Function to remove animation styles when animation ends
  ...
  navLinks.forEach((link) => {
    link.addEventListener("click", () => {
      closeNav();
      setTimeout(removeAnimation, 1000); // Delay removal of animation styles
    });
  });
...
const init = () => {
  ...
  letters = [];
  const chars = ["S", "A", "R", "A", "V", "A","N","A","N"];
  const colors = ["#02d2d6", "#b125d1", "#fea064", "#00a8e8", "#fb89ef","orange"];
